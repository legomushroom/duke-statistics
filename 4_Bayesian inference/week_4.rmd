...
<ol>
<li> Observational study </li> <!-- This: Observational study -->
<li> Experiment </li>
</ol>
...
<li> 7 of the respondents make less than 300 dollars per week </li> <!-- This: actually 6 of them -->
...
wage %>% filter(wage < mean(wage)) %>% summarise(n = n())
wage %>% filter(wage >= mean(wage)) %>% summarise(n = n())
```
...
ggplot(wage, aes(x = wage, y = iq)) + geom_point()
ggplot(wage, aes(x = wage, y = hours)) + geom_point()
```
...
<li> No, since the distribution of the residuals of the model is right-skewed.  </li> <!-- This -->
</ol>

```{r}
# type your code for Question 3 here, and Knit
# type your code for the Question 5 here, and Knit
#plot(m_wage_iq$residuals ~ wage$iq) # linear assosiation = random scatter

#plot(m_wage_iq$residuals ~ m_wage_iq$fitted) # constant variability - no fan shape
#plot(abs(m_wage_iq$residuals) ~ m_wage_iq$fit)

hist(m_wage_iq$residuals) # nearly normal residuals - left skewed
qqnorm(m_wage_iq$residuals)
qqline(m_wage_iq$residuals)

#plot(m_wage_iq$residuals) # independence / time series - no pattern
...
<li> (0.00709, 0.01050) </li> <!-- This: (0.00709, 0.01050) -->
...
<ol>
<li> The married black man </li> <!-- This -->
...
m_lwage_test = lm(lwage ~ . -wage -sibs, data = na.omit(wage))
BIC(m_lwage_test)

m_lwage_test = lm(lwage ~ . -wage -feduc, data = na.omit(wage))
BIC(m_lwage_test)

m_lwage_test = lm(lwage ~ . -wage -meduc, data = na.omit(wage))
BIC(m_lwage_test)
```
...
```{r stepBIC}
# type your code for the Exercise here, and Knit
non_na_wages = na.omit(wage)
m_AIC = lm(lwage ~ . -wage, data = non_na_wages)
result_model = stepAIC(m_AIC, k = log(length(non_na_wages)))
BIC(result_model)
```
...
par(mfrow = c(1,1))
# type your code for the Exercise here, and Knit
coef_red = coefficients(wage_red_m)
summary(coef_red)
plot(coef_red, subset = c(8), ask=FALSE)
```
...
quantile(y_5, probs = seq(0.025, 0))
quantile(y_5, probs = seq(0.975, 1))
```
