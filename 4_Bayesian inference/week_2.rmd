...
print(qnorm(c(0.025, 0.975), mean = 10, sd = 2.236))
```
...
```{r beta-ci}
# type your code for the Exercise here, and Knit
print(qbeta(c(0.05, 0.95), shape1 = 2, shape2 = 5))
```
...
```{r gamma-ci}
# type your code for the Exercise here, and Knit
print(qgamma(c(0.001, 0.999), shape = 4, rate = 8))
```
...
<li> The probability that the true proportion of females lies in this interval is 0.95.</li> <!-- THIS -->
...
data = c(2, 3, 4, 5, 4)
a_star = a + sum(data)
b_star = b + length(data)
print(paste(a_star, b_star, sep = ", "))
```
...
<li> Gamma($a = 8119$, $b = 5001$) </li> <!-- THIS -->
...
a_star = a + sum(brfss$fruit_per_day)
b_star = b + length(brfss$fruit_per_day)

print(paste(a_star, b_star))
```
...
a_star = a + sum(brfss$vege_per_day)
b_star = b + length(brfss$vege_per_day)

print(paste(a_star, b_star))

qgamma(c(0.005, 0.995), shape = a_star, rate = b_star)
```
