...
<ol>
<li> Left skewed </li> <!-- This: Left skewed -->
<li> Right skewed </li>
...
<ol>
<li> (7.00 , 7.19) </li>
<li> (6.98 , 7.22) </li> <!-- This: (6.98 , 7.22) -->
<li> (6.94 , 7.26) </li>
<li> (6.94 , 7.27) </li>
</ol>
...
```{r}
# type your code for the Exercise here, and Knit
bayes_inference(data = nc, y = weight, statistic = "mean", null = 5.5, type="ht", alternative = "twosided")
```
...
```{r normal-ci}
# type your code for the Exercise here, and Knit
nc2 <- nc %>% filter(!is.na(habit)) %>% filter(!is.na(weight))
ggplot(nc2, aes(x = habit, y = weight)) + geom_boxplot()

ggplot(nc2 %>% filter(habit == "nonsmoker"), aes(x = weight)) + geom_histogram()
ggplot(nc2 %>% filter(habit == "smoker"), aes(x = weight)) + geom_histogram()
```
...
<li> Both distributions are extremely right skewed. </li> <!-- This: Both distributions are extremely right skewed. -->
<li> The IQRs of the distributions are roughly equal. </li>
</ol>
```{r habit-weight-box}
# type your code for Question 5 here, and Knit
"<!-- This: Both distributions are extremely right skewed. -->"
```
...
<ol>
<li> Not worth a bare mention </li> <!-- This: Not worth a bare mention -->
<li> Positive </li>
<li> Strong </li>
<li> Very Strong </li>
</ol>
...
<li> Get bigger </li> 
<li> Get smaller </li>
<li> Stay the same </li> <!-- This: Stay the same -->
</ol>

```{r}
bayes_inference(y = lowbirthweight, data = nc, success = "low", statistic = "proportion", type = "ht", null = 0.075, alternative = "twosided", hypothesis_prior = c(.25, .75))
```

9. How would the Bayes factor above change if we were to change the prior of p to be $Beta(75,925)$?
<ol>
<li> Get bigger </li>
<li> Get smaller </li> <!-- This: Get smaller -->
<li> Stay the same </li>
</ol>
...
```{r}
#nc2 <- nc %>% filter(nc$premie == "premie")
bayes_inference(y = weight, x = habit, data = nc, statistic = "mean", type = "ht", null = 0, alternative = "twosided")
```

10. These data provide ___weak____ evidence ____against___ smoking affecting the chance of low birth weight.
<ol>
<li> weak; for </li>
<li> strong; for </li>
<li> weak; against </li> 
...
```{r}
#nc2 <- nc %>% filter(nc$lowbirthweight == "low")
bayes_inference(y = weeks, x = habit, data = nc, statistic = "mean", type = "ht", null = 0, alternative = "twosided")
```

11. These data provide __week___ evidence ___against___ smoking affecting the chance of premature birth.
<ol>
<li> weak; for </li>
<li> strong; for </li>
<li> weak; against </li> 
