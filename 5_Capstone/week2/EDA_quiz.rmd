...
<li> `Misc.Feature`, `Alley`, `Pool.QC` </li> <!-- This: `Misc.Feature`, `Alley`, `Pool.QC` -->
<li> `Pool.QC`, `Alley`, `Fence` </li> 
<li> `Fireplace.Qu`, `Pool.QC`, `Lot.Frontage` </li>
</ol>
```{r Q1}
# type your code for Question 1 here, and Knit
sum(colSums(is.na(ames_train[, c('Misc.Feature', 'Fence', 'Pool.QC')])))
sum(colSums(is.na(ames_train[, c('Misc.Feature', 'Alley', 'Pool.QC')])))
sum(colSums(is.na(ames_train[, c('Pool.QC', 'Alley', 'Fence')])))
sum(colSums(is.na(ames_train[, c('Fireplace.Qu', 'Pool.QC', 'Lot.Frontage')])))
```
...
ames_train2 <- ames_train[,c("Misc.Feature","Fence", "Pool.QC", "Alley", "Fireplace.Qu", "Lot.Frontage")]

for(name in names(ames_train2)){
  
  print(name)
  type = class(ames_train2[, name])
  
  if (type == "integer") {
    print('  Integer: yes')
    smry = summary(as.factor(ames_train2[ ,name]))
    print(str(smry))
  } else {
    print('  Integer: no')
  }
...
neibourhoods = c("StoneBr", "Timber", "Veenker", "NridgHt")

ames_train %>%
    group_by(Neighborhood) %>%
    filter(Neighborhood %in% neibourhoods) %>%
...
<li> `Lot.Area`
<li> `Bedroom.AbvGr`
<li> `Overall.Qual` <!-- This: Overall.Qual -->
<li> `Year.Built`
</ol>

```{r Q4}
# type your code for Question 4 here, and Knit
v = c('Lot.Area', 'Bedroom.AbvGr', 'Overall.Qual', 'Year.Built')
...
<li> Log-transform both `price` and `area` <!-- This: Both -->
</ol>
...
<li> Beta(963, 46)
<li> Beta(954, 47)
<li> Beta(963, 47) <!-- This: Beta(963, 47) -->
</ol>

```{r Q6}
# type your code for Question 6 here, and Knit
x = count(ames_train %>% filter(!is.na(Garage.Type)))
n = count(ames_train)
print(paste(x, n))
# Beta(9 + 954, 1 + 1000 - 954)
```
...
<li> 21 houses do not have a basement. <!-- This: 21 no basement -->
<li> 4 houses are located on gravel streets.
</ol>

```{r Q7}
# type your code for Question 7 here, and Knit
first = count(ames_train %>% filter(Year.Built > 1999)) / count(ames_train)
second = summary(ames_train$price)
third = count(ames_train %>% filter(is.na(Bsmt.Qual)))
print(first)
print(second)
print(third)
```
...
<ol>
<li> With a p-value near 0.000, we reject the null hypothesis of no difference. <!-- This: p-value == 5.773316e-06  -->
...
with = ames_train %>% filter(!is.na(Garage.Type))
without = ames_train %>% filter(is.na(Garage.Type))

with_mean = mean(with$area)
without_mean = mean(without$area)
with_sd = sd(with$area)
without_sd = sd(without$area)

print(paste(with_mean, without_mean, with_sd, without_sd))

# H0: µ1 - µ2 = 0
# Ha: µ1 - µ2 != 0

pooled_SE = sqrt( (with_sd^2 / count(with)) + (without_sd^2 / count(without)) )
df = min( count(with) - 1, count(without) -1 )

#print(pooled_SE)

T_score = ((with_mean - without_mean) - 0) / pooled_SE

#print(T_score$n)
#print(df)

p_value = 2*pt(T_score$n, df = df, lower.tail = FALSE)
...
greater = ames_train %>% filter(area > 2000)
