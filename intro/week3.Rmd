...
# kobe_streak %>% summarize(median = median(length), IQR = IQR(length))
```
coin
...
densKobe <- density(as.vector(t(kobe_streak)))
densSim <- density(as.vector(t(sim_streak)))
## calculate the range of the graph
xlim <- range(densSim$x,densKobe$x)
ylim <- range(0,densSim$y, densKobe$y)
#pick the colours
kobeCol <- rgb(1,0,0,0.2)
simCol <- rgb(0,0,1,0.2)
## plot the carrots and set up most of the plot parameters
plot(densKobe, xlim = xlim, ylim = ylim, xlab = 'Lengths',
     main = 'Distribution of Kobe and Simulated', 
     panel.first = grid())
#put our density plots in
polygon(densKobe, density = -1, col = kobeCol)
polygon(densSim, density = -1, col = simCol)
## add a legend in the corner
legend('topright',c('Kobe','Simulation'),
       fill = c(kobeCol, simCol), bty = 'n',
       border = NA)
