...
nycflights <- nycflights %>% mutate(type_fix = ifelse((arr_type == "on time" & dep_type == "delayed"), 1, 0))

nycflights %>% group_by(dep_type) %>% summarise(n = n(), n_fix = sum(type_fix), frac= sum(type_fix)/n())
