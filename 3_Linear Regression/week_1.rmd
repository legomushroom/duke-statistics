...
<li> The relationship is positive, linear, and very weak. One of the potential outliers is a team with approximately 5520 at bats. </li> <!-- THIS -->
</ol>
...
plot_ss(data = mlb11, x = at_bats, y = runs)
```
...
plot_ss(data = mlb11, x = at_bats, y = runs, showSquares = TRUE)
# 123721.9 123721.9
```
...
<ol>
<li> For each additional home run, the model predicts 1.83 more runs, on average. </li> <!-- THIS -->
...
\[
  \hat{y} = 415.24 + 1.834 \times homeruns 
\]
...
predicted = -2789.2429 + (0.6305*5579)
residual = 713 - predicted
print(residual)
```
...
<li> The residuals show a curved pattern. </li>  <!-- THIS -->
...
<li> The residuals are fairly symmetric, with only a slightly longer tail on the right, hence it would be appropriate to deem the the normal distribution of residuals condition met. </li> <!-- THIS -->
</ol>
...
ggplot(data = m3, aes(x = .resid)) + geom_histogram(binwidth = 25) + xlab("Residuals")
```
...
k1 = lm(runs ~ at_bats, data = mlb11)
k2 = lm(runs ~ hits, data = mlb11)
k3 = lm(runs ~ wins, data = mlb11)
k4 = lm(runs ~ bat_avg, data = mlb11)
# Rsquared is 0.6561 as appose to the 0.37
```
...
ggplot(data = k1, aes(sample = .resid)) + stat_qq()
ggplot(data = k1, aes(x = .resid)) + geom_histogram(binwidth = 25)

ggplot(data = k1, aes(x = .fitted, y = .resid)) +
  geom_point() + 
  geom_hline(yintercept = 0, linetype = "dashed")
...
ggplot(data = k2, aes(sample = .resid)) + stat_qq()
ggplot(data = k2, aes(x = .resid)) + geom_histogram(binwidth = 25)

ggplot(data = k2, aes(x = .fitted, y = .resid)) +
  geom_point() + 
  geom_hline(yintercept = 0, linetype = "dashed")
...
ggplot(data = k3, aes(sample = .resid)) + stat_qq()
ggplot(data = k3, aes(x = .resid)) + geom_histogram(binwidth = 25)

ggplot(data = k3, aes(x = .fitted, y = .resid)) +
  geom_point() + 
  geom_hline(yintercept = 0, linetype = "dashed")
...
ggplot(data = k4, aes(sample = .resid)) + stat_qq()
ggplot(data = k4, aes(x = .resid)) + geom_histogram(binwidth = 25)

ggplot(data = k4, aes(x = .fitted, y = .resid)) +
  geom_point() + 
  geom_hline(yintercept = 0, linetype = "dashed")
...
l1 <- lm(runs ~ new_obs, data = mlb11)
l2 <- lm(runs ~ new_slug, data = mlb11)
l3 <- lm(runs ~ new_onbase, data = mlb11)
```
```{r}
ggplot(data = mlb11, aes(x = new_obs, y = runs)) + geom_point()

ggplot(data = l1, aes(sample = .resid)) + stat_qq()
ggplot(data = l1, aes(x = .resid)) + geom_histogram(binwidth = 10)

ggplot(data = l1, aes(x = .fitted, y = .resid)) +
  geom_point() + 
  geom_hline(yintercept = 0, linetype = "dashed")
...
ggplot(data = l2, aes(sample = .resid)) + stat_qq()
ggplot(data = l2, aes(x = .resid)) + geom_histogram(binwidth = 25)
...
ggplot(data = l3, aes(sample = .resid)) + stat_qq()
ggplot(data = l3, aes(x = .resid)) + geom_histogram(binwidth = 25)

ggplot(data = l3, aes(x = .fitted, y = .resid)) +
  geom_point() + 
  geom_hline(yintercept = 0, linetype = "dashed")
