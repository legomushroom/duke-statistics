...
On the plot we can see a strong positive about linear assosiation between the variables. We can also see a slight curvature in the data, we will need to confirm the linearity condition.
...
We expect to see exactly random scatter around `0` to confirm the `linearity` condition, the condition does not strongly  holds, many observations clustered in bottom right corner hence we will need to clearly state the reservation in the conclusion of our study.
...
Unfortunately we crearly see the negative fan shape for in the plot hnce the condition does not hold too.
...
We see a slight right skew in the histogram, with such a big sample it might be totally fine. Looking at the theoretical quantiles plot we see few outliers at the higher end, while the deviation might be strong these are just few observations so we can conclude that the condition holds with few reservations.
...
We have a large `adjusted R^2` value which evaluates or model performance. As we expected the `p-values` for the `best_actor_win`, `best_dir_win` and `best_actress_win` are much larger than the significance level of `5%` thus we will reject the null hypothesis concluding that thees data do not provide significant evidence that each of the variables are significant predictors of the response variable, given all else hold constant. Of course since we use the adjusted R^2 method, we will not use this insight for model optimization.
...
We can see that the `F-statistic` for the model is `895.3` on 3 and 646 degrees of freedom, such a large critical F score gives us nearly zero `p-value` will confirms the model performace for such big `adjusted R^2` value.

Alos we can see that each of the variables have the tiny `p-values` which states that they are significant predictors of the response(`imdb_rating`) variable.

## int

* Interpret slopes
* make the CIs and interpret them
