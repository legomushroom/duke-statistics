...
The question is of interest for the author since when he searches for a movie to watch, he have to read the rating on both `IMDB` and `Rotten Tomatoes` resources. Knowing a relation between rating of the two sites and by building a linear regression model, the author can potentially save a lot of time by using the model to predict expected rating on the `imdb` resource by reading just by visiting `Rotten Tomatoes` website, instead of reading thru both `imdb` and `Rotten Tomatoes` sites.
...
Now we can pick a movie form the past to make a prediction. It is important to pick a movie that was released inbetween 1970-2014, as otherwise the prediction will imply `extrapolation` which will give us unreliable results.
...
So we are `95%` confident that the true `The Lord of the Rings: The Two Towers (2002)` movie rating on `imdb` for audience score equal to 95%, with runtime of 180minutes and critics score = 96% is 7.5 to 9.4 points.

Our model predicted the value to be `8.46` with the uncertainty interval above. If we will refere to the moview at the the [imdb](http://www.imdb.com/title/tt0167261/?ref_=nv_sr_3) site, we will see that the model slighly underestimates the rating, it is very close to the real observed value though, given that we use just 3 linear predictors:
...
We have praticed the theoretical knowlage we gained during the course by a research over the observational movies data sample that represents a random sample of 651 observations collected starting 1970 to up to 2014. We have analyzed the variables the data have and understood correlation between them, found the variables that might be useful for building a `multivariate linear regression model` for predicting `idmb_rating` score with `audience_score`, `runtime` `critics_score` variables. For a sanity check, we took a new movie and successfully predicted its `idmb_rating`.

The most noticable findings see can conclude:

- Reviewers on the `IMDB` tend to leave a good rating, there seem to be a very little observations with ration less than 5.5 points.
- Reviewers on the `Rotten Tomatoes` though, have much more diversity of opinions, the observetions distribution was much more spread, the right end of the distribution was much closer to the higher end (100 score).
- Surprisingly for the author, whether actor/actress or director have previously had a high valued regalia or not, the fact doesn't seem to have a large corrlation on the `IMDB` rating.
- Even a small amount of predictors can help to make a good predictive model - we had just 3 explanatory variables in the final model and the model performed pretty well, given that the methods we could use are constratined to linear ones.
- Movie's runtime variable slightly improved the performance of the model which is not expected by the author.
- While `audience_score` and `critics` score had a high correlation with each other which has to complicate model estimation, the model performace increased after including bith variables which might not always be the case.

### Reservations

The final regression model performed pretty well on a single "test" sample but it might not be tru if we have had a "sample" of the test examples to evaluate the model on, while the movie was picked randomly, the good performace we observed might be simply due to chance - we should have a "test sample" to test our model on for better understanding of model's performance.

When we made a model diagnostics, we allowed a couple of reservations:

- While the `audience_score` vs `imdb_rating` scatterplot look nearly linear there is deffinetely a minor curvature on the plot, further examination of the fact could be done for determining what effect it can have on the study.
- During confirming independence condition, all 3 variable have some outliers present, there were just few of them, they definetely affect the model performance in the linear regression setting.
- Constant variability condition plot with absolute residuals have a set of outliers in the left top corner, 
which might speak to shape of the destribution to be fan out, while we don't believe that is the case, we need to exclude such possibility.
- The residuals normality could be checked in more detail, due to moderate skeweness we have observed.
