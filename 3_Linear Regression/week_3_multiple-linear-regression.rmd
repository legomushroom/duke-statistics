...
<ol>
<li> Observational study </li> <!-- THIS -->
<li> Experiment </li>
</ol>
...
<li> Yes, revise wording to "Is there an association between beauty and course evaluations?" </li> <!-- THIS -->
...
<ol>
<li> The median of the distribution is 4.3. </li> <!-- TRUE -->
<li> 25\% of the students gave their professors a score of over 4.6. </li> <!-- TRUE -->
<li> 11 of students gave a professor a score below 3. </li> <!-- TRUE -->
<li> The left skewness of the data suggests that the students are less likely to rate the professors highly. </li> <!-- FALSE -->
</ol>
```{r score-dist}
# type your code for Question 3 here, and Knit
#ggplot(evals, aes(x = score)) + geom_histogram()

#evals %>% group_by(score) %>% summarize(n = n())
...
```{r jitter}
# type your code for the Exercise here, and Knit
ggplot(evals, aes(x = bty_avg, y = score)) + geom_jitter()
```
...
ggplot(evals, aes(x = bty_avg, y = score)) + geom_jitter() + geom_smooth(method = "lm")
...
ggplot(evals, aes(x = bty_avg, y = score)) + geom_jitter() + geom_smooth(method = "lm", se = FALSE)
```
...
<ol>
<li> Linear association: The residuals plot shows a random scatter. </li> <!-- TRUE -->
<li> Constant variance of residuals: No fan shape in residuals plot. </li> <!-- TRUE -->
<li> Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions. </li> <!-- FALSE -->
<li> Independent observations: Classes sampled randomly, no order effect. </li> <!-- TRUE -->
</ol>
```{r diag-score-bty_avg}
# type your code for the Question 5 here, and Knit
plot(model$residuals ~ evals$bty_avg) # linear assosiation = random scatter

plot(model$residuals ~ model$fitted) # constant variability - no fan shape
plot(abs(model$residuals) ~ model$fit)

hist(model$residuals) # nearly normal residuals - left skewed
qqnorm(model$residuals)
qqline(model$residuals)

plot(model$residuals) # independence / time series - no pattern
```
...
ggplot(evals, aes(x = bty_f1lower, y = bty_avg)) + geom_jitter()
...
```{r diag-mlr-model}
# type your code for the Exercise here, and Knit
plot(m_bty_gen$residuals ~ evals$gender) # linear assosiation = random scatter

plot(m_bty_gen$residuals ~ m_bty_gen$fitted) # constant variability - no fan shape
plot(abs(m_bty_gen$residuals) ~ m_bty_gen$fit)

hist(m_bty_gen$residuals) # nearly normal residuals - left skewed
qqnorm(m_bty_gen$residuals)
qqline(m_bty_gen$residuals)

plot(m_bty_gen$residuals) # independence / time series - no pattern
```
...
<li> Tenure Track, Tenured, Teaching </li> <!-- THIS -->
...
<li> 0.12 points higher than minority professors, all else held constant. </li> <!--THIS-->
