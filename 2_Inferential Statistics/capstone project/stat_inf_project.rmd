...
With such a small `p-value` we will `reject the null-hypothesis` and conclude that the data provides significant evidence that indeed there is a relation between TV watching time habits and job satisfaction variables.
...
As we can see the test statistics and all other parameters are almost equal to what we had, so it converged to the same results.