...
To uderstand the actuall differences about to levels we can compare each TV watching time level by job satisfaction with other, reulsting in `(k * (k - 1)) / 2` total comparations. But first we will need to check all the condition needed for the inference over two categorical variables (same for each pair):

1. Independence
  - Whithin groups
    - random sampling indeed
    - sampling without replacing, < 10% of entire population
  - Between groups - the observations are independent between gropus - we assume that the TV watching habits and job satisfaction is independent among the people in the sample
...
H0: p1 = p2 # the true poppulation parameter is the same among groups
Ha: p1 != p2 # the true poppulation parameter is NOT the same among groups
...
compareTwoProportions = function(contingencyTable, i1, i2) {
  writeLines("\n-=-=-=-=-=-=-=-=-=-=-=-")
  columnNames = colnames(contingencyTable)
  cat("Comparing to proporiotns: '", columnNames[i1], "' and '",  columnNames[i2], "' ", sep = "")
  # get the columns
  ...
  if ((CIlower > 0 & CIupper > 0) | (CIlower < 0 & CIupper < 0)) {
    cat("\n✔ Statisticly sighnificant difference in proportions: ", "we'd reject the null hypoothesis CI(", CIlower, ", ", CIupper, ")", " SE: ", SE, sep = "" )
  } else {
    cat("\n⊗ Not a statisticly sighnificant difference in proportions: ", "we'd not reject the null hypoothesis CI(", CIlower, ", ", CIupper, ")", " SE: ", SE, sep = "" )
  }
  ...
  if (pvalue < .05) {
    cat("\n✔ [hypothesis test] Statisticly sighnificant difference in proportions: ", "we'd reject the null hypoothesis pvalue = ", pvalue, " SE: ", SE, sep = "" )
  } else {
    cat("\n⊗ [hypothesis test] Statisticly sighnificant difference in proportions: ", "we'd not reject the null hypoothesis pvalue = ", pvalue, " SE: ", SE, sep = "" )
  }
}
...
for (i in k:(nColumns-2)) {
  for (j in (k+1):(nColumns-1)) {
    #cat("\ni: ", i, " j: ", j, sep="")
    compareTwoProportions(contingency, i, j)
  }
  k = k + 1
}
...
As we can see from the results of running the function on each pair, we were able to find the significant evidence in difference of population proportions in all but 2 pairs ( '0 not watching' <-> '2 watching'  and '3 watching a lot' <-> '4 all time'). Also the CIs and hypothesis tests agreed on results in all but the last pair('3 watching a lot' <-> '4 all time' ), since we use proportion estimation(phat_pool) for hypothesis tests, it might result in not accurate `standart error` estimation as we had with CI tests: `0.01850337`(ci) vs `0.01580229`(ht) thus resulting in slightly different conclusions. This is might be expected as the CI lower bound is very close to zero, and speaking in the context of the data, the difference between '3 watching a lot' and '4 all time' levels is somewhat on the border of the significance level threshold.


### Recap

We conducted the statistical inference in the context the question of interest: "Does there appear to be a relationship between time spent in front of TV and job satisfaction level?". We used the `chi-square test of independece` as we wanted to find an association between two variables one of which contain more than two levels. We have found a statisticly significant difference in proportions and concluded dependence. After that we looked closer on the data to find what levels exactly do have statisticly significant difference and found out that the strong majority of the groups do have the difference. The `CIs` and `hypothesis` tests agreed in all cases but the last one - due to the `standart error` estimation difference, and we assumed that the CI estimation is more accurate.
