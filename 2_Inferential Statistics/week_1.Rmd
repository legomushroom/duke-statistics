...
<li> 50\% of houses in Ames are smaller than 1,499.69 square feet. </li> <!-- This -->
...
ggplot(data = sample1, aes(x = area)) + geom_histogram(binwidth = 250)
sample1 %>% summarise(x = mean(area), s = sd(area), min = min(area), max = max(area), iqr = IQR(area), p1 = quantile(area, .25), p3 = quantile(area, .75))
```
...
<li> Sample size of 1000. </li> <!-- This -->
</ol>
...
ggplot(data = sample_means_small, aes(x = x_bar)) + geom_histogram(binwidth = 100)
```
...
<ol>
<li> Each element represents a mean square footage from a simple random sample of 10 houses. </li> <!-- This -->
...
ames %>% sample_n(size = 50) %>% summarize(x_bar = mean(price))
```
...
```{r price-sampling}
# type your code for this Exercise here, and Run Document
sample_means50 <- ames %>% rep_sample_n(size = 50, reps = 5000, replace = TRUE) %>% summarise(x_bar = mean(price), s = sd(price))
ggplot(data = sample_means50, aes(x = x_bar)) + geom_histogram(binwidth = 5000)
```
...
ggplot(data = sample_means150, aes(x = x_bar)) + geom_histogram(binwidth = 5000)
```
...
# type your code for this Exercise here, and Run Document
ames %>% sample_n(size = 15, replace = TRUE) %>% summarise(x_bar = mean(price), s = sd(price))
```
...
# type your code for this Exercise here, and Run Document
sample2000 <- ames %>% rep_sample_n(size = 15, reps = 2000, replace = TRUE) %>% summarise(x_bar = mean(price))

print(sample2000 %>% summarise(xx_bar = mean(x_bar), x_sd = sd(x_bar)))

ggplot(data = sample2000, aes(x = x_bar)) + geom_histogram(binwidth = 4000)
```
...
# type your code for this Exercise here, and Run Document
sample_150_2000 <- ames %>% rep_sample_n(size = 150, reps = 2000, replace = TRUE) %>% summarise(x_bar = mean(price), sd = sd(price))
print(sample_150_2000 %>% summarise(xx_bar = mean(x_bar), x_sd = sd(sd)))

ggplot(data = sample_150_2000, aes(x = x_bar)) + geom_histogram(binwidth = 2000)
```

6. Which of the following is false? 
<ol>
<li> The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`). </li> <!-- This -->
