...
<li> A poll conducted by WIN-Gallup International surveyed 51,927 people from 57 countries. </li> <!-- This  -->
</ol>
...
us12 %>% group_by(response) %>% summarise(n = n()/nrow(us12), total = nrow(us12))
```
...
<li> The margin of error for the estimate of the proportion of atheists in the US in 2012 is 0.025. </li> <!-- THIS  -->
...
ua12 <- atheism %>% filter( nationality == "Ukraine", year == "2012" )
tr12 <- atheism %>% filter(nationality == "Turkey", year == "2012")

inference( y = response, data = tr12, method = "theoretical", type = "ci", statistic = "proportion", success = "atheist")

inference( y = response, data = ua12, method = "theoretical", type = "ci", statistic = "proportion", success = "atheist")
```
...
<li> The $ME$ is maximized when $p = 0.5$. </li> <!-- THIS -->
...
spD <- atheism %>% filter(nationality == "Spain", year == c("2012", "2005"))

spD %>% group_by(year, response) %>% summarise(n = n())

inference(response, x = year, method = "theoretical", type = "ht", data = spD, statistic = "proportion", success = "atheist", alternative = "twosided", sig_level = 0.05)
```
...
usD <- atheism %>% filter(nationality == "United States", year == c("2012", "2005"))

usD %>% group_by(year, response) %>% summarise(n = n())

inference(response, x = year, method = "theoretical", type = "ht", sig_level = 0.005, success = "atheist", statistic = "proportion", data = usD, alternative = "twosided")
...
<li> 9604 people </li> 
<li> At least 9604 people </li> <!-- THIS -->
</ol>
```{r sample-size}
